    ...
    static class PostsHandler implements HttpHandler {
        ...
        private void handleGetComments(HttpExchange exchange) throws IOException {
            ...
            if (postIdOpt.isPresent()) {
                ...
                if (post.isPresent()) {
                    List<Comment> comments = post.get().getCommentaries();
                ...
                String errorResponse = "Некорректный идентификатор поста";
                writeResponse(exchange, errorResponse, 400);
            }
        }
