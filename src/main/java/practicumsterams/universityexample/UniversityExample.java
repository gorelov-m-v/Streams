package practicumsterams.universityexample;
...
public class UniversityExample {
    ...
    public static void main(String[] args) {
        ...
        List<Student> graduatedStudents = students.stream()
                ...
                .map(student -> {
                    if (student.entranceYear == 2021) {
                        student.groupName = groupNames.get(2021);
                    } else {
                        student.groupName = groupNames.get(2020);
                    }
                    return student;
                })
                // операция peek выполняет над элементом действие и передаёт далее тот же элемент
                .peek(student -> graduatesClub.add(student.toString()))
